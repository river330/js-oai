...
async function main() {
  ...
  const context = [];
  let playing = true;
  const location = "a secret sea temple";
  const player = {};
  player.name = await ask("What is your name?");
  player.class = await ask("What type of sea creature are you?");
  ...
  while (playing) {
    ...
    const prompt = `
  This is a text adventure game set underwater. There is currently a divide between the King Sharks and the Mermaid Kingdom in which our player is currently in the middle of.
